...
# ═══════════════════════════════════════════════════════════════
#  GHOST ENTRY  — versión corregida
#  El placeholder se borra SIEMPRE al hacer foco, aunque el
#  campo ya tenga texto (evita el bug de "escribir encima").
# ═══════════════════════════════════════════════════════════════
def make_ghost_entry(parent, placeholder, width=28, **kwargs):
    ...
    def on_focus_in(event):
        # Siempre limpiar si el placeholder sigue ahí
    ...
    def on_focus_out(event):
        # Solo restaurar placeholder si el campo está vacío
        if not e.get().strip():
            e.delete(0, "end")
...
def ghost_get(entry):
    """Retorna el valor real (vacío si es placeholder)."""
    if entry._has_placeholder:
        return ""
    return entry.get().strip()

def ghost_clear(entry):
    """Resetea el entry a estado placeholder."""
    entry.delete(0, "end")
...
def ghost_set(entry, value):
    """Pone un valor real en el entry (quita el placeholder)."""
    entry.delete(0, "end")
    if value:
...
# ═══════════════════════════════════════════════════════════════
#  PANEL PORTERÍA  — igual al historial de administrador
#  con búsqueda automática al escribir
# ═══════════════════════════════════════════════════════════════
class PanelPorteria(tk.Frame):
    def __init__(self, master, root=None, close_callback=None):
        ...
        self.pack(fill="both", expand=True)
        self._search_after_id = None  # para debounce de búsqueda automática
        self._build()
        self._auto_refresh()
    ...
    def _build(self):
        ...
        # ── Filtro de búsqueda (igual que en historial) ──────────────
        ...
        self.e_buscar = make_ghost_entry(left, "Ej. Juan Pérez", width=34)
        ...
        # Mismas columnas que el historial de administrador
    ...
    def _on_key_buscar(self, event=None):
        """Debounce: espera 300 ms tras la última pulsación antes de filtrar."""
    ...
    def _limpiar_filtro(self):
        ghost_clear(self.e_buscar)
        self._refresh()

    def _refresh(self):
        filtro = ghost_get(self.e_buscar)
        ...
        # Obtener todos los movimientos de hoy, agrupados por par Ingreso/Salida
        q = """
...
# ═══════════════════════════════════════════════════════════════
#  GESTIÓN DE ESTUDIANTES  — búsqueda automática al escribir
# ═══════════════════════════════════════════════════════════════
class GestionEstudiantes(tk.Frame):
    ...
    def _build(self):
        ...
        # Búsqueda automática al escribir (debounce 300 ms)
        self.e_buscar.bind("<KeyRelease>", self._on_key_buscar)
...
# ═══════════════════════════════════════════════════════════════
#  CONSULTA HISTORIAL  — búsqueda automática al escribir
# ═══════════════════════════════════════════════════════════════
class ConsultaHistorial(tk.Frame):
    ...
    def _build(self):
        ...
        # Búsqueda automática al escribir la fecha
    ...
    def _on_key_buscar(self, event=None):
        """Debounce 300 ms para no buscar en cada tecla."""
        if self._search_after_id is not None:
            try:
                self.after_cancel(self._search_after_id)
            except Exception:
    ...
    def _buscar(self):
        ...
        params = []
        if nombre:
            # Búsqueda flexible: por nombre completo O nombre O apellido por separado
        ...
        # No mostrar popup cuando no hay resultados durante escritura en tiempo real
        # (solo si el usuario hizo búsqueda vacía mostramos todo)
