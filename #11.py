...
def make_header(parent, title):
    hdr = tk.Frame(parent, bg=BG_HEADER, height=58)
    hdr.pack(fill="x")
    hdr.pack_propagate(False)
    tk.Label(hdr, text=title, bg=BG_HEADER,
             font=("Segoe UI", 15, "bold"), padx=18).pack(side="left", pady=0)
    return hdr

def make_navbar(parent, items):
    nav = tk.Frame(parent, bg="#c5cad1", height=46)
    nav.pack(fill="x")
    nav.pack_propagate(False)          # mantener altura fija
...
# ─────────────────────────────────────────
#  NOTIFICACIÓN FLOTANTE (toast 3 segundos)
# ─────────────────────────────────────────
def mostrar_toast(master, mensaje, color=COLOR_GREEN, duracion=3000):
    """
    Muestra una ventana flotante sin bordes durante `duracion` ms,
    centrada en la ventana raíz.
    """
    # Obtener la ventana raíz real
    root = master.winfo_toplevel()

    toast = tk.Toplevel(root)
    toast.overrideredirect(True)          # sin barra de título
    toast.attributes("-topmost", True)
    toast.configure(bg=color)

    lbl = tk.Label(toast, text=mensaje, bg=color, fg="white",
                   font=("Segoe UI", 13, "bold"), padx=28, pady=16)
    lbl.pack()
...
def realizar_registro_db(est_id, nombre, apellido, master=None):
    """
    Determina si el próximo movimiento es Ingreso o Salida,
    SIEMPRE crea un registro nuevo (nunca edita existentes),
    muestra toast si master está disponible,
    y retorna (tipo, nombre, apellido).
    """
    now   = datetime.now()
    ...
    # Determinar el tipo: si el ÚLTIMO registro de HOY es Ingreso → Salida, si no → Ingreso
    c.execute(
    ...
    # SIEMPRE insertar un nuevo registro (nunca UPDATE)
    c.execute(
...
# ═══════════════════════════════════════════════════════════════
#  VENTANA DE RECONOCIMIENTO FACIAL — AUTO-REGISTRO
#  El programa reconoce el rostro y lo registra automáticamente
#  sin que el usuario tenga que pulsar ningún botón.
#  Muestra toast de confirmación y luego cierra la cámara.
# ═══════════════════════════════════════════════════════════════
class VentanaCamaraReconocimiento(BaseWindow):
    """
    Mejoras respecto a la versión original:
    - Auto-registro: en cuanto se reconoce un rostro con suficiente
      confianza durante N frames consecutivos, lo registra y cierra.
    - Ya no es necesario pulsar "Confirmar Registro".
    - Muestra una barra de progreso visual mientras confirma el rostro.
    - Toast de 3 s al completar.
    """
    FRAMES_CONFIRMACION = 8   # cuántos frames seguidos reconocidos para registrar

    def __init__(self, master, callback_registro):
        super().__init__(master, "Reconocimiento Facial", "750x580")
        self.callback_registro = callback_registro
        self.capturando        = True
        self.rostro_reconocido = None
        self.imgtk_ref         = None
        self._frames_confirm   = 0     # contador de frames con el mismo rostro
        self._registrando      = False # evitar doble registro
        self._build()
    ...
    def _build(self):
        ...
        self.lbl_video = tk.Label(self, bg="black")
        self.lbl_video.pack(padx=10, pady=(10,4))
        ...
        prog_frame.pack(pady=4)
        tk.Label(prog_frame, text="Confirmando:", bg=BG_MAIN, font=FONT_SMALL,
                 fg=COLOR_GRAY).pack(side="left", padx=(0,6))
        self.progress = ttk.Progressbar(prog_frame, length=260, maximum=self.FRAMES_CONFIRMACION,
                                        mode="determinate")
        self.progress.pack(side="left")

        # Botón manual como respaldo
        make_btn(self, "✔ Confirmar manualmente", self._confirmar_manual,
                 color=COLOR_GRAY, width=24).pack(pady=6)
    ...
    def _loop_video(self):
        ...
        while self.capturando:
            ...
            if frame_count % 3 == 0:
                ...
                # Auto-registrar cuando alcanza el umbral
    ...
    def _auto_registrar(self):
        """Llamado desde el hilo principal cuando se confirman suficientes frames."""
...
class PantallaInicio(tk.Frame):
    ...
    def _abrir_camara(self):
        try:
            # El callback aquí solo sirve para refrescar paneles si hay alguno abierto;
            # el registro ya lo hace VentanaCamaraReconocimiento internamente.
    ...
    def _after_camara(self, est_id, nombre, apellido):
        """
        Callback post-cámara. El registro ya fue creado en VentanaCamaraReconocimiento;
        aquí sólo se muestra la pantalla de éxito.
        """
        # Consultar el tipo del último registro insertado para mostrarlo
        conn = get_connection()
        c = conn.cursor()
...
class PantallaRegistroExitoso(BaseWindow):
    ...
    Muestra cuenta regresiva en el botón para que el usuario sepa que va a cerrarse.
    """
    SEGUNDOS = 3

    def __init__(self, master, nombre, apellido, tipo):
        super().__init__(master, "Registro Exitoso", "540x360")
        ...
        tk.Label(self, text=emoji_tipo, font=("Segoe UI", 52),
                 bg=BG_MAIN, fg=color_tipo).pack(pady=(28, 4))
        tk.Label(self, text="¡Registro Exitoso!", bg=BG_MAIN,
                 font=("Segoe UI", 16, "bold")).pack()

        card = tk.Frame(self, bg=BG_CARD, bd=1, relief="solid", padx=30, pady=18)
        card.pack(pady=16)
        ...
        self.btn_cerrar = make_btn(
            ...
            self.destroy, color=COLOR_GRAY, width=20
        )
    ...
    def _tick(self):
        """Descuenta un segundo; al llegar a 0 cierra la ventana."""
...
def open_new_window(master, cls, root=None):
    ...
    win.geometry("1100x700")          # ← ventanas más amplias
...
# ═══════════════════════════════════════════════════════════════
#  PANEL PORTERÍA — con tabla de registros individuales
#  Cada movimiento aparece en su propia fila (sin editar anteriores)
# ═══════════════════════════════════════════════════════════════
class PanelPorteria(tk.Frame):
    ...
    def _build(self):
        ...
        make_btn(top, "🔄 Actualizar", self._refresh, color=COLOR_GRAY, width=14).pack(side="right")

        card = tk.Frame(self, bg=BG_CARD, bd=1, relief="solid", padx=16, pady=16)
        card.pack(fill="both", expand=True, padx=20, pady=(0,20))

        tk.Label(card, text="Registros individuales de movimientos",
                 bg=BG_CARD, font=FONT_HEADER).pack(anchor="w", pady=(0,8))

        # Tabla: cada fila = un movimiento independiente
        cols = ("ID", "Estudiante", "Tipo", "Fecha", "Hora")
        self.tree = ttk.Treeview(card, columns=cols, show="headings", height=16)
        widths = {"ID": 55, "Estudiante": 280, "Tipo": 110, "Fecha": 130, "Hora": 110}
        for col in cols:
    ...
    def _refresh(self):
        ...
        conn = get_connection()
        c = conn.cursor()
        # Cada registro es una fila independiente, ORDER BY id DESC = más reciente primero
        c.execute("""
            SELECT r.id, e.nombre||' '||e.apellido, r.tipo, r.fecha, r.hora
            FROM registros r
            JOIN estudiantes e ON r.estudiante_id = e.id
            ORDER BY r.id DESC
            LIMIT 100
        """)
        for reg_id, nombre, tipo, fecha, hora in c.fetchall():
            tag = "ingreso" if tipo == "Ingreso" else "salida"
            self.tree.insert("", "end", iid=reg_id,
                             values=(reg_id, nombre, tipo, fecha, hora),
                             tags=(tag,))
        conn.close()
        # Colores por tipo
    ...
    def _auto_refresh(self):
        """Refresca la tabla cada 5 segundos automáticamente."""
        try:
            self._refresh()
...
class GestionEstudiantes(tk.Frame):
    ...
    def _build(self):
        ...
        widths = {"ID":70,"Nombre":220,"Apellido":220,"Grado":100,"Rostro":120}
        for col in cols:
            self.tree.heading(col, text=col)
        ...
        self.tree.configure(yscrollcommand=sb.set)
        self.tree.pack(side="left", fill="both", expand=True, padx=(20,0), pady=(0,16))
        sb.pack(side="left", fill="y", pady=(0,16), padx=(0,20))
...
# ═══════════════════════════════════════════════════════════════
#  CONSULTA HISTORIAL — registros individuales por movimiento
# ═══════════════════════════════════════════════════════════════
class ConsultaHistorial(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(right, text="Fecha (AAAA-MM-DD):", bg=BG_CARD, font=FONT_NORMAL).pack(anchor="w")
        self.e_fecha = tk.Entry(right, font=FONT_NORMAL, width=22, bd=1, relief="solid")
        self.e_fecha.pack(pady=4, ipady=5)

        make_btn(filt, "Buscar", self._buscar, width=10).pack(side="left", padx=20, pady=16)

        res = tk.Frame(self, bg=BG_CARD, bd=1, relief="solid", padx=16, pady=14)
        res.pack(fill="both", expand=True, padx=20, pady=(0,16))
        tk.Label(res, text="Movimientos individuales", bg=BG_CARD, font=FONT_HEADER).pack(anchor="w", pady=(0,8))

        # Columnas: cada fila = un movimiento (no se colapsan ingresos/salidas)
        cols = ("ID", "Estudiante", "Tipo", "Fecha", "Hora")
        self.tree = ttk.Treeview(res, columns=cols, show="headings", height=14)
        widths = {"ID": 60, "Estudiante": 300, "Tipo": 120, "Fecha": 140, "Hora": 120}
        for col in cols:
            self.tree.heading(col, text=col)
            self.tree.column(col, width=widths[col], anchor="center" if col != "Estudiante" else "w")
    ...
    def _buscar(self):
        ...
        c.execute(q, params)
        for row in c.fetchall():
            reg_id, est, tipo, fch, hora = row
            tag = "ingreso" if tipo == "Ingreso" else "salida"
            self.tree.insert("","end", values=(reg_id, est, tipo, fch, hora), tags=(tag,))
        conn.close()


# ═══════════════════════════════════════════════════════════════
#  GENERACIÓN DE REPORTES — muestra cada movimiento por separado
# ═══════════════════════════════════════════════════════════════
class GeneracionReportes(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(filt, text="Tipo:", bg=BG_CARD, font=FONT_NORMAL).grid(row=2,column=0,sticky="w",pady=(10,0))
        self.combo_tipo = ttk.Combobox(filt, width=18,
                                        values=["Todos","Solo Ingresos","Solo Salidas"])
        self.combo_tipo.current(0)
        self.combo_tipo.grid(row=2,column=1,padx=(6,0),pady=(10,0),sticky="w")
        make_btn(filt,"Generar Reporte",self._generar,width=18).grid(
        ...
        widths = {"ID":60,"Fecha":130,"Estudiante":280,"Tipo":120,"Hora":110}
        for col in cols:
            self.tree.heading(col,text=col)
            self.tree.column(col,width=widths[col], anchor="center" if col != "Estudiante" else "w")
        ...
        make_btn(self,"Volver al panel",self.winfo_toplevel().destroy,width=18).pack(
    ...
    def _generar(self):
        ...
        if ":" in rango:
            ...
            if len(partes) == 2 and partes[0] != "AAAA-MM-DD":
...
def main():
    ...
    style.configure("Treeview", background=BG_WHITE, foreground="#333",
                    rowheight=30, fieldbackground=BG_WHITE, font=FONT_NORMAL)
    style.configure("Treeview.Heading", background=BG_HEADER, foreground="#333",
                    font=FONT_HEADER, padding=(8,6))
