...
def make_navbar(parent, items):
    ...
    for label, cmd in items:
        btn = tk.Button(nav, text=label, command=cmd, bg=BG_HEADER, fg="#333",
                  font=FONT_NORMAL, relief="flat", cursor="hand2", padx=14, pady=6)
        btn.pack(side="left")
        # Hover effect para navbar
        btn.bind("<Enter>", lambda e, b=btn: b.config(bg="#c5cad1"))
        btn.bind("<Leave>", lambda e, b=btn: b.config(bg=BG_HEADER))
    return nav
