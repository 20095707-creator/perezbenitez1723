...
class PantallaInicio(tk.Frame):
    ...
    def _build(self):
        ...
        make_btn(card, "🔍 Identificación por cámara",
                 self._abrir_camara, color=COLOR_GREEN, width=30).pack(pady=10)
    ...
    def _registrar(self):
        ...
        self._realizar_registro(est_id, nombre, apellido)
    ...
    def _abrir_camara(self):
        try:
            ...
            messagebox.showerror("Error de cámara",
                                 f"No se pudo iniciar la cámara: {e}")
...
# ═══════════════════════════════════════════════════════════════
#  VENTANA DE RECONOCIMIENTO FACIAL EN TIEMPO REAL (OPTIMIZADA)
# ═══════════════════════════════════════════════════════════════
class VentanaCamaraReconocimiento(BaseWindow):
    ...
    def _build(self):
        ...
        self.lbl_info = tk.Label(self, text="Buscando rostro...",
                                 bg=BG_MAIN, font=FONT_HEADER)
        self.lbl_info.pack(pady=5)

        make_btn(self, "Confirmar Registro", self._confirmar,
                 color=COLOR_GREEN, width=22).pack(pady=5)
    ...
    def _loop_video(self):
        ...
        while self.capturando:
            ...
            if process_this_frame:
                ...
                face_locations = face_recognition.face_locations(rgb_small, number_of_times_to_upsample=0)
                ...
                for encoding in face_encodings:
                    if self.known_encodings:
                        ...
                        if True in coincidencias:
                            ...
                            est_id = self.known_ids[idx]
                            nombre = self.known_names[idx]
                            nombre_mostrar = nombre
                            self.rostro_reconocido = (est_id, nombre)
                            color = (0, 255, 0)
                            break
            ...
            self.lbl_video.after(0, self._update_video_label, imgtk)

            if self.rostro_reconocido:
                self.lbl_info.after(0, lambda: self.lbl_info.config(
                    text=f"✅ Reconocido: {self.rostro_reconocido[1]}", fg="green"))
            else:
                self.lbl_info.after(0, lambda: self.lbl_info.config(
...
# ═══════════════════════════════════════════════════════════════
#  VENTANA DE CAPTURA DE ROSTRO (segura para hilos)
# ═══════════════════════════════════════════════════════════════
class CapturaRostro(BaseWindow):
    def __init__(self, master, callback):
        super().__init__(master, "Capturar Rostro", "450x400")
    ...
    def _build(self):
        ...
        btn_frame.pack()
        tk.Button(btn_frame, text="Capturar", command=self._capturar,
                  font=FONT_BTN, bg=COLOR_GREEN, fg="white",
                  padx=20, pady=8).pack(side="left", padx=10)
        tk.Button(btn_frame, text="Cancelar", command=self._cancelar,
                  font=FONT_BTN, bg=COLOR_RED, fg="white",
    ...
    def _loop(self):
        ...
        while self.capturando:
            ...
            # Dibujar rectángulo en frame de vista previa
            ...
            img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            imgtk = ImageTk.PhotoImage(image=img)
            self.lbl_video.after(0, self._update_frame, imgtk)
    ...
    def _capturar(self):
        ...
        if frame is not None:
            ...
            if encs:
                ...
                messagebox.showinfo("Capturado", "Rostro capturado. Ahora pulse Guardar para almacenarlo.")
                self.callback(self.face_encoding)
                self._cerrar()
            else:
                messagebox.showwarning("Sin rostro", "No se detectó ningún rostro. Intente de nuevo.")
        else:
            messagebox.showwarning("Error", "No hay imagen disponible aún.")
...
# ═══════════════════════════════════════════════════════════════
#  PANTALLA 6 – GESTIÓN DE ESTUDIANTES (con guardado claro)
# ═══════════════════════════════════════════════════════════════
...
class FormEstudiante(BaseWindow):
    def __init__(self, master, est_id, callback):
        super().__init__(master, "Formulario Estudiante", "420x500")
        self.est_id   = est_id
        self.callback = callback
        self.face_encoding = None
        self._build()
    ...
    def _build(self):
        ...
        self.btn_capturar = make_btn(card, "📸 Capturar Rostro",
                                     self._abrir_captura, color=COLOR_GRAY, width=20)
        self.btn_capturar.grid(row=4, column=0, columnspan=2, pady=12)

        if self.est_id:
            conn = get_connection()
            c = conn.cursor()
            c.execute("SELECT nombre, apellido, grado, password FROM estudiantes WHERE id=?",
                      (self.est_id,))
            row = c.fetchone()
            conn.close()
            if row:
        ...
        # Al cerrar la ventana, verificar si hay codificación sin guardar
        self.protocol("WM_DELETE_WINDOW", self._on_close)

    def _abrir_captura(self):
        CapturaRostro(self.winfo_toplevel(), self._recibir_encoding)

    def _recibir_encoding(self, encoding):
        if encoding is not None:
            self.face_encoding = encoding
            self.btn_capturar.config(text="✅ Rostro capturado", bg=COLOR_GREEN)
    ...
    def _guardar(self):
        ...
        # Guardar codificación facial si existe
        ...
        if self.face_encoding is not None:
            try:
                ...
                messagebox.showerror("Error", f"No se pudo guardar el rostro:\n{e}")
                conn.rollback()
                conn.close()
                return
        ...
        if rostro_guardado:
            msg += " También se guardó la codificación facial."
        messagebox.showinfo("Guardado", msg)

        self.face_encoding = None  # ya guardado, evitar advertencia al cerrar
        self.callback()
        self.destroy()

    def _on_close(self):
        # Si hay una codificación capturada pero no guardada, advertir
        if self.face_encoding is not None:
            if not messagebox.askyesno("Rostro no guardado",
                                       "Has capturado un rostro pero no lo has guardado. ¿Cerrar de todos modos?"):
                return
        self.destroy()
