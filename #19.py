...
# ═══════════════════════════════════════════════════════════════
# CAMBIO 5: make_navbar con botones más visibles (fondo azul oscuro,
# texto blanco, tamaño mayor) para Admin y Portería.
# Se usa make_navbar_panel para esos paneles y make_navbar normal
# para las ventanas internas (Volver).
# ═══════════════════════════════════════════════════════════════
...
def make_navbar_panel(parent, items):
    """Navbar con botones más visibles: fondo azul, texto blanco, fuente grande."""
    nav = tk.Frame(parent, bg="#1a73e8", height=60)
    nav.pack(fill="x")
    nav.pack_propagate(False)
    for label, cmd in items:
        btn = tk.Button(
        ...
        btn.pack(side="left")
        btn.bind("<Enter>", lambda e, b=btn: b.config(bg="#1558b0"))
        btn.bind("<Leave>", lambda e, b=btn: b.config(bg="#1a73e8"))
    return nav
...
class CapturaRostro(BaseWindow):
    ...
    # CAMBIO 1: eliminado messagebox.showinfo de _capturar
    def _capturar(self):
        with self.lock:
        ...
        self.update()
        # CAMBIO 1: se eliminó messagebox.showinfo("Éxito", ...)
...
class FormEstudiante(BaseWindow):
    ...
    # CAMBIO 1: eliminado messagebox.showinfo("Guardado", ...) con rostro_msg
    def _guardar(self):
        ...
        try:
            ...
            conn.commit()
            # CAMBIO 1: eliminado messagebox.showinfo con rostro_msg; solo se cierra
            self.face_encoding = None
            self.callback()
            self.destroy()
...
class PantallaInicio(tk.Frame):
    ...
    def _build(self):
        ...
        # CAMBIO 2: Entry normal sin ghost (sin placeholder) en PantallaInicio
...
# ═══════════════════════════════════════════════════════════════
#  PANEL PORTERÍA
# CAMBIO 5: navbar con make_navbar_panel (más visible)
# CAMBIO 5: eliminado botón "Cerrar sesión" del navbar
# CAMBIO 2: entry de búsqueda sin ghost (Entry normal sin placeholder)
# ═══════════════════════════════════════════════════════════════
class PanelPorteria(tk.Frame):
    ...
    def _build(self):
        make_header(self, "RAE")
        # CAMBIO 5: navbar más visible, sin "Cerrar sesión"
        ...
        # CAMBIO 2: Entry normal sin ghost en portería
...
class PanelAdministracion(tk.Frame):
    ...
    def _build(self):
        make_header(self, "RAE")
        # CAMBIO 5: navbar más visible para administrador
        make_navbar_panel(self, [
            ("🏠  Inicio", self._ir_inicio),
        ])
...
# CAMBIO 4: ORDER BY e.id ASC (orden numérico)
# ═══════════════════════════════════════════════════════════════
class GestionEstudiantes(tk.Frame):
    ...
    def _cargar_todos(self, filtro=""):
        ...
        conn = get_connection()
        c = conn.cursor()
        # CAMBIO 4: ORDER BY e.id ASC para orden numérico
        query = """
...
# ═══════════════════════════════════════════════════════════════
#  CONSULTA HISTORIAL
# CAMBIO 2: entries sin ghost (Entry normal sin placeholder)
# ═══════════════════════════════════════════════════════════════
class ConsultaHistorial(tk.Frame):
    ...
    def _build(self):
        ...
        # CAMBIO 2: Entry normal sin ghost en historial
