...
def ghost_clear(entry):
    """Reset a ghost entry back to its placeholder state."""
    entry.delete(0, "end")
...
class PantallaInicio(tk.Frame):
    ...
    def _build(self):
        ...
        # Ghost entry for ID — clears automatically after successful registration
        self.entry_id = make_ghost_entry(card, "Número de Identificación", width=36)
    ...
    def _registrar(self):
        num_id = ghost_get(self.entry_id)
        if not num_id:
        ...
        if not row:
            ...
            return
        # Clear field automatically after successful lookup
        ghost_clear(self.entry_id)
...
# ═══════════════════════════════════════════════════════════════
#  PANEL PORTERÍA  (sin pestaña Monitoreo; incluye Historial)
# ═══════════════════════════════════════════════════════════════
class PanelPorteria(tk.Frame):
    ...
    def _build(self):
        make_header(self, "RAE")
        # Navbar: only Inicio, Historial, Cerrar sesión (no Monitoreo)
        make_navbar(self, [
    ...
    def _refresh(self):
        ...
        for est_id, nombre, entrada, salida in c.fetchall():
            ...
            if not ultimo or ultimo[0] is None:
                ...
                estado = "🚪 Salió"
                tag = "fuera"
            self.tree.insert("", "end", iid=str(est_id),
                             values=(nombre, entrada_txt, salida_txt, estado), tags=(tag,))
        conn.close()
...
# ═══════════════════════════════════════════════════════════════
#  PANEL ADMINISTRACIÓN  (sin "Cerrar sesión"; sin "Monitoreo")
# ═══════════════════════════════════════════════════════════════
class PanelAdministracion(tk.Frame):
    ...
    def _build(self):
        make_header(self, "RAE")
        # Only "Inicio" — no "Monitoreo", no "Cerrar sesión"
        make_navbar(self, [
            ("Inicio", self._ir_inicio),
        ])
...
class GestionEstudiantes(tk.Frame):
    ...
    def _build(self):
        ...
        self.e_buscar = make_ghost_entry(search_card, "Buscar por nombre o ID", width=70)
...
# ═══════════════════════════════════════════════════════════════
#  CONSULTA HISTORIAL  (con botón Eliminar Registro integrado)
#  Aparece tanto en Administración como en Portería
# ═══════════════════════════════════════════════════════════════
class ConsultaHistorial(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(top, text="Consulta de Historial",
                 bg=BG_MAIN, font=FONT_TITLE).pack(side="left")
        # Eliminar Registro button lives here now
        ...
        self.e_nombre.bind("<Return>", lambda e: self._buscar())
        ...
        self.e_fecha.bind("<Return>", lambda e: self._buscar())

        tk.Label(filt, text="(Sin fecha = todo el historial)",
                 bg=BG_CARD, font=("Segoe UI", 10), fg=COLOR_GRAY).pack(side="left", padx=20)

        btn_buscar = make_btn(filt, "Buscar", self._buscar, width=12)
        btn_buscar.pack(side="left", padx=20, pady=16)

        # Results + clear-filter button
        ...
        # Store raw data for deletion lookup (keyed by tree iid)
        self._row_meta = {}  # iid -> (nombre_est, fecha)
    ...
    def _buscar(self):
        ...
        # grupos: (nombre_est, fecha, est_id) -> list of (tipo, hora, reg_id)
    ...
    def _limpiar_filtros(self):
        ghost_clear(self.e_nombre)
        ghost_clear(self.e_fecha)
        self._buscar()

    def _eliminar_registro(self):
        """Elimina el último registro (más reciente en esa fecha) de la fila seleccionada."""
        ...
        if last_reg_id:
            ...
            # Fallback: delete most recent record for that student on that date
            c.execute("""
...
# ═══════════════════════════════════════════════════════════════
#  GENERACIÓN DE REPORTES  (unchanged)
# ═══════════════════════════════════════════════════════════════
